...
public class JwApiFactory extends ApiUtils {
    public static final int MERGE_SCHEDULE = 0;
    public static final int SPLIT_SCHEDULE = 1;
//    private JwApi service;
    private JwApiFactory() {
        super();
//        password = AppConfig.jwPwd;
    }
    ...
    public void  getSchedule(String stu_time,int split,Observer< List<Schedule> > sub) {
        ApiUtils.getApi(AppConfig.jwPwd).create(JwApi.class).getSchedule(stu_time,split)
    ...
    public void  getBasicInfo(Observer<BasicInfo> sub) {
        ApiUtils.getApi(AppConfig.jwPwd).create(JwApi.class).getBasicInfo()
